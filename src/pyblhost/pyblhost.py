...
class BlhostBase(ABC):
            ...
            try:
                ...
                if not upload_result:
                    ...
                    if not self._flash_erase_region_response_event.wait(timeout):
                        self.logger.error(
                            "BlhostBase: Timed out waiting for flash erase region response after the upload failed"
                        )
            finally:
    ...
    def _upload(
        ...
        for d in self.chunks(binary_data, 32):
            ...
            data_sent += len(d)
            yield data_sent / len(binary_data) * 100.0
    ...
    def read(
        ...
        while True:
            yield len(self._memory_data) / byte_count * 100.0
    ...
    def __init__(
        ...
        if can_bus is None:
            ...
            can_filters = [
                {
                    "can_id": self._tx_id,
                    "can_mask": 0x1FFFFFFF if self._extended_id else 0x7FF,
...
def cli() -> None:
    ...
    parser.add_argument(
        "command",
        help="upload: write BINARY to START_ADDRESS. Before writing it will erase the "
    ...
    optional.add_argument(
        "-t", "--timeout", help="The time to wait in seconds for a response (default 1.0)", default=1.0, type=float
    )
    optional.add_argument(
        "-r",
        "--cmd-repeat",
        help="The number of times to try to establish a connection (default 3)",
        default=3,
        type=int,
    )
    optional.add_argument(
        ...
        "--baudrate",
        "--bitrate",
        help="The baudrate/bitrate to use for serial/can (default 500000)",
        type=int,
        default=500000,
    )
